...
# Real-arm MoveIt on VcXsrv: planning here (WSL), execution on the Atlas board.
# While WSLg is broken (GPU-PV dxg failures), reuse the VcXsrv display.
...
exec ros2 launch so101_moveit_config real_arm.launch.py
