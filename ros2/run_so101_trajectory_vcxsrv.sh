...
# SO-ARM101 trajectory visualization (project 2) on VcXsrv instead of WSLg.
# Same VcXsrv display setup as run_so101_display_vcxsrv.sh, but launches
# so101_visualization/trajectory_viz.launch.py which additionally starts the
# end-effector trajectory recorder (ee_trajectory_recorder.py) and RViz2 with
# the trajectory view (/ee_path + /ee_trail + live stats).
...
# VcXsrv display settings (same as the display/MoveIt VcXsrv scripts).
...
exec ros2 launch so101_visualization trajectory_viz.launch.py
